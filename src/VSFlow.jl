...
function profilerun(p::Profile, accfunc; animate=false, is4thorder=true, bdorder=4)
    ...
    while(count <= 1 + Int(p.T/p.dt))
        ...
        if animate && count%skip == 0
            ...
            pp = scatter(x, y, zcolor=-Γ, markercolor=palette(:RdBu_4), markersize=1.8,
                          markerstrokewidth=0, aspect_ratio=1, xlim=(-.5-scale*p.T, scale),
                          ylim=(-2scale, 2scale),
                          framestyle=:none,
                          grid=false,
                          ticks=false,
            ...
            # plot!(pp, xp, yp, line=(1, :black), fill=(0, RGB{Float64}(96/255,123/255,163/255)), legend=false)
            plot!(pp, xp, yp, line=(1, :black), legend=false)

            frame(anim)

            # (abs(p.timelapse - 10.) < 1e-5) && savefig(p.fname*"short.svg")
        end
    end
